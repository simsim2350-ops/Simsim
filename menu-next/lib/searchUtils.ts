...
// Ported verbatim from src/features/menu/searchUtils.js (the old menu's real, live search
// ranking algorithm) — pure logic, no framework dependency, so the exact same behavior
// carries over unchanged. Do not re-derive a different scoring scheme here.
...
// ترتيب الأصناف حسب جودة المطابقة، ثم مختارات المطعم، ثم Best Sellers اليدوية.
export function rankProducts(query: string, products: Product[], bestSellerIds?: Set<string>): Product[] {
  return products
    .map((p) => ({ p, s: scoreProduct(query, p) }))
    .filter((x) => x.s > 0)
    .sort((a, b) => {
      if (b.s !== a.s) return b.s - a.s
      const aFeatured = !!a.p.is_featured, bFeatured = !!b.p.is_featured
