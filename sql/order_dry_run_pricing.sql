-- Phase 3 / TASK-PAY-3.6A-1a (proposed — pending owner confirmation before Production application)
-- الهدف: إضافة قدرة "تسعير جاف" (dry-run) لدالة create_order الحالية، بحيث تستطيع خدمة الدفع
-- المستقبلية (Payment-First Checkout, غير مبنية بعد) الحصول على السعر الإجمالي المُحتسَب من الخادم
-- قبل إنشاء أي محاولة دفع — دون إدخال أي طلب فعلي ودون استهلاك أي كوبون.
--
-- القرار المعماري: Option D من reports/TASK_3_6A_1_SERVER_SIDE_PRICING_QUOTE_ARCHITECTURE_AUDIT.md —
-- إعادة استخدام نفس مسار حساب التسعير الحالي حرفياً (بلا استخلاص، بلا دالة منفصلة، بلا جدول عروض
-- أسعار)، مع نقطة عودة مبكرة واحدة قبل أي تعديل دائم في قاعدة البيانات.
--
-- NOT APPLIED TO PRODUCTION YET. هذا الملف مقترح فقط — بانتظار موافقة المالك/DBA، وبانتظار تحقق ناجح
-- على Staging أولاً (rgqsetckcigkgsyobyjg)، طبقاً لتعليمات المهمة الصريحة.
--
-- Rollback: DROP FUNCTION public.create_order(uuid, uuid, text, text, text, text, text, jsonb, text,
-- text, numeric, uuid, uuid, boolean); ثم إعادة إنشاء التوقيع السابق (13 معامل، بلا p_dry_run) من
-- sql/order_payment_reference.sql (الجسم غير متأثر بهذا الملف على الإطلاق سوى بإضافة معامل واحد
-- وفرع IF واحد).
--
-- ⚠️ نفس التحذير المحفوظ من sql/order_idempotency.sql وsql/order_payment_reference.sql:
-- CREATE OR REPLACE FUNCTION بقائمة معاملات مختلفة لا يستبدل الدالة — يُنشئ Overload جديداً. لذلك هذا
-- الملف يُسقط صراحةً التوقيع الحالي (13 معامل، مُتحقَّق منه حيّاً عبر pg_get_function_identity_arguments
-- قبل كتابة هذا الملف) قبل إنشاء الجديد (14 معامل).
--
-- التغيير الوحيد في جسم الدالة، حرفياً، هو:
--   (1) معامل جديد: p_dry_run boolean DEFAULT false (توافق خلفي كامل — أي مستدعٍ حالي لا يرسل هذا
--       المعامل يستمر بالعمل تماماً كما هو، لأن القيمة الافتراضية false تحافظ على السلوك الحالي حرفاً
--       بحرف).
--   (2) شرط واحد إضافي على مسار الإرجاع المبكر الحالي لـ idempotency_key: أصبح
--       "if p_idempotency_key is not null and not p_dry_run then" بدل "if p_idempotency_key is not
--       null then" — لمنع تسريب معرّف طلب حقيقي سابق (id/order_number/access_token) في استجابة
--       يُفترض أنها "تسعير جاف بلا طلب" في حال تصادف تمرير مفتاح idempotency يخص طلباً موجوداً فعلاً.
--   (3) فرع IF جديد واحد فقط، يُدرَج بعد فحص price_changed الحالي (الذي يبقى كما هو تماماً — بلا أي
--       تعديل) وقبل تحديث usage_count الخاص بالكوبون مباشرة: إن كان p_dry_run=true، تُعاد نفس القيم
--       المحسوبة (subtotal/tax/delivery_fee/total) بنفس اصطلاح NULL المستخدم أصلاً لحقول
--       id/order_number/access_token في مسار price_changed — دون إدخال أي صف في orders ودون تحديث
--       usage_count. لا تكرار لأي صيغة حسابية (خصم/ضريبة/توصيل/خيارات) — نفس المتغيرات المحسوبة أعلاه
--       بالضبط تُعاد كما هي.
--
-- لا شيء آخر تغيّر: منطق التحقق من المطعم/الفرع/المنتجات/الخيارات/الكوبون (بما فيها قفل FOR UPDATE على
-- صف الكوبون) بقي حرفياً كما هو في sql/order_payment_reference.sql.

DROP FUNCTION IF EXISTS public.create_order(uuid, uuid, text, text, text, text, text, jsonb, text, text, numeric, uuid, uuid);

CREATE OR REPLACE FUNCTION public.create_order(p_restaurant_id uuid, p_branch_id uuid, p_table_number text, p_delivery_address text, p_customer_name text, p_customer_phone text, p_type text, p_items jsonb, p_notes text, p_coupon_code text, p_client_total numeric DEFAULT NULL::numeric, p_idempotency_key uuid DEFAULT NULL::uuid, p_payment_transaction_id uuid DEFAULT NULL::uuid, p_dry_run boolean DEFAULT false)
...
begin
  ...
  -- طلب حقيقي سابق، حتى لو صودف أن مفتاح idempotency الممرَّر يخص طلباً موجوداً فعلاً.
  ...
  -- أثر دائم. id/order_number/access_token = NULL بنفس اصطلاح مسار price_changed أعلاه: لا يوجد طلب
  -- فعلي وراء هذه النتيجة. يجب أن يُدرَج هذا الفرع هنا تحديداً — بعد اكتمال كل الحسابات، وقبل أي
  -- تعديل دائم (تحديث usage_count الكوبون، إدخال صف في orders).
  if p_dry_run then
