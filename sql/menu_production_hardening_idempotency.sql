-- Simsim Menu Production Hardening
-- Server-authoritative order creation, secure order access, limited realtime payloads.
-- Apply after the existing menu/branch/marketing migrations.
...
create unique index if not exists orders_order_access_token_uidx
...
alter table public.coupons
  drop constraint if exists coupons_usage_count_check;
alter table public.coupons
  add constraint coupons_usage_count_check check (usage_count >= 0);
alter table public.coupons
  drop constraint if exists coupons_max_discount_amount_check;
alter table public.coupons
  add constraint coupons_max_discount_amount_check check (max_discount_amount is null or max_discount_amount >= 0);

-- The legacy client-trusted signature is absent on a fresh staging schema.

alter table public.orders add column if not exists idempotency_key text;
create unique index if not exists orders_restaurant_idempotency_key_uq
  on public.orders (restaurant_id, idempotency_key) where idempotency_key is not null;
...
begin
  ...
  insert into public.orders (
    ...
    v_access_token, v_idempotency_key
...
returns table(id uuid, order_number text, status text, cancelled_by text, items jsonb, total numeric, updated_at timestamptz)
language sql
security definer
set search_path = public
as $function$
...
returns table(id uuid, status text, cancelled_by text, cancelled_at timestamptz)
language sql
security definer
set search_path = public
as $function$
...
revoke all on function public.can_read_order_status(text) from public;
grant execute on function public.can_read_order_status(text) to anon, authenticated;
...
begin
  ...
  perform realtime.broadcast(
    'order-status:' || new.id::text || ':' || new.order_access_token,
    tg_op,
    jsonb_build_object(
      'order_id', new.id,
      'status', new.status,
...
using (
  ...
  and public.can_read_order_status(realtime.topic())
);

notify pgrst, 'reload schema';


-- Secure trigger search_path without changing trigger behavior.
