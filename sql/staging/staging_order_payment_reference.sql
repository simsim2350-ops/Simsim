...
-- ════════════════════════════════════════════════════════════════════════════
-- Task 3.5 staging variant. This is NOT sql/order_payment_reference.sql (the
...
-- (its own inline idempotency-key length validation, p_idempotency_key TEXT
-- not uuid) — this file preserves that body exactly and adds ONLY the new
-- payment-reference parameter/logic on top of it.
--
-- Depends on sql/staging/staging_payments_gateway_foundation.sql having been
-- applied first (needs public.payment_transactions to exist for the FK).
--
-- Rollback:
--   DROP FUNCTION public.create_order(uuid, uuid, text, text, text, text, text, jsonb, text, text, numeric, text, uuid);
--   -- then recreate the original 12-arg staging function (body captured verbatim
--   -- in reports/STAGING_TARGETED_PAYMENT_PARITY_PLAN.md's "CURRENT STAGING STATE"
--   -- section, fetched live via pg_get_functiondef before this file was written);
--   ALTER TABLE public.orders DROP COLUMN payment_transaction_id; -- drops FK + unique index too
-- ════════════════════════════════════════════════════════════════════════════
...
begin
  ...
  if v_args ~ 'p_payment_transaction_id' then
    raise exception 'STAGING GUARD: create_order already has p_payment_transaction_id — already applied. Aborting to avoid re-running.';
  end if;

  if to_regclass('public.payment_transactions') is null then
    raise exception 'STAGING GUARD: public.payment_transactions does not exist — apply sql/staging/staging_payments_gateway_foundation.sql first. Aborting.';
  end if;
end;
$guard$;
...
-- 2) قيد فريد جزئي: محاولة دفع واحدة ⇒ طلب واحد على الأكثر.
CREATE UNIQUE INDEX orders_payment_transaction_id_uidx
  ON public.orders (payment_transaction_id)
  WHERE payment_transaction_id IS NOT NULL;

-- 3) قطع نظيف: إسقاط التوقيع الحالي (12 معامل، p_idempotency_key TEXT) قبل إنشاء الجديد (13 معامل).
DROP FUNCTION IF EXISTS public.create_order(uuid, uuid, text, text, text, text, text, jsonb, text, text, numeric, text);

-- create_order — جسم Staging الفعلي الحالي (مُجلَب حرفياً عبر pg_get_functiondef قبل كتابة هذا
-- الملف) + معامل ثالث عشر جديد بقيمة افتراضية NULL + منطق التحقّق من مرجع الدفع فقط. لم يُستبدَل
-- الجسم بجسم Production — يُحافَظ على فحص طول مفتاح الإتقان الخاص بـStaging كما هو.
CREATE OR REPLACE FUNCTION public.create_order(p_restaurant_id uuid, p_branch_id uuid, p_table_number text, p_delivery_address text, p_customer_name text, p_customer_phone text, p_type text, p_items jsonb, p_notes text, p_coupon_code text, p_client_total numeric DEFAULT NULL::numeric, p_idempotency_key text DEFAULT NULL::text, p_payment_transaction_id uuid DEFAULT NULL::uuid)
