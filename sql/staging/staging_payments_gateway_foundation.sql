...
-- ════════════════════════════════════════════════════════════════════════════
-- Targeted payment-parity work, approved scope only (see
-- reports/STAGING_TARGETED_PAYMENT_PARITY_PLAN.md and
-- reports/STAGING_TARGETED_PAYMENT_PARITY_EXECUTION_REPORT.md).
--
-- Content mirrors sql/payments_gateway_foundation.sql (the production-targeted
-- file, which is NOT modified by this work). Applied here because staging's
-- `invoices`/`subscriptions` tables already exist as valid FK targets, unlike
-- a from-scratch sandbox — confirmed by read-only inspection before writing
-- this file.
--
-- Rollback (additive-only change, safe to reverse in this order):
...
begin
  ...
  if v_args is null then
    raise exception 'STAGING GUARD: create_order not found in this database — aborting (unexpected shape, this file is staging-only).';
  end if;

  if v_args !~ 'p_idempotency_key text' then
    raise exception 'STAGING GUARD: create_order.p_idempotency_key is not "text" in this database — this file is STAGING-ONLY and must not run against a database shaped like Production (which uses uuid). Aborting.';
  end if;

  if to_regclass('public.payment_transactions') is not null then
    raise exception 'STAGING GUARD: public.payment_transactions already exists — this file appears to have already been applied. Aborting to avoid re-running.';
  end if;
end;
$guard$;
