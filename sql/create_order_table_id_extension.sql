-- Backward-compatible extension: adds an optional p_table_id parameter to
-- create_order for the branch-URL "select your table from a real list"
-- flow. When provided, it is validated against restaurant_tables (must
-- belong to the SAME restaurant_id/branch_id and be status='active') before
-- being trusted — a customer can never claim a table_id from another
-- branch/restaurant. The table's own real table_number is used for the
-- order row (never the client-supplied p_table_number string) once a
-- table_id is given, so the invoice always shows the authoritative name for
-- that id. When p_table_id is omitted (every pre-existing caller), behavior
-- is byte-for-byte identical to before: v_final_table_number falls back to
-- the same nullif(trim(coalesce(p_table_number,''))) expression already
-- used.
--
...
--
-- Verified before applying: the only live caller of create_order is
-- menu-next/components/CheckoutForm.tsx (the old menu's own useCheckout.js
-- call is dead code, since /menu/:slug redirects client-side to menu-next
-- before ever mounting it; the payment-first orchestration layers' calls
-- were already confirmed unwired/unreachable). create_order_from_table_qr's
-- own internal call passes 12 named arguments and does not reference
-- p_table_id at all, so it is completely unaffected.
--
-- Existing execute grants (including to PUBLIC) are preserved exactly as
-- they were before this migration.
drop function if exists public.create_order(uuid, uuid, text, text, text, text, text, jsonb, text, text, numeric, uuid, uuid, boolean);
...
create or replace function public.create_order(
  ...
  p_payment_transaction_id uuid DEFAULT NULL::uuid,
...
begin
  ...
  -- p_table_id, when given, must be a real, active table belonging to THIS
  -- exact restaurant+branch — never trusted blindly, never allowed to
...
revoke execute on function public.create_order(uuid, uuid, text, text, text, text, text, jsonb, text, text, numeric, uuid, uuid, boolean, uuid) from public;
grant execute on function public.create_order(uuid, uuid, text, text, text, text, text, jsonb, text, text, numeric, uuid, uuid, boolean, uuid) to public, anon, authenticated, postgres, service_role;
