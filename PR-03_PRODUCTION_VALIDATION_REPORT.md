...
**3) بعد موافقة المالك: نُفِّذ الخيار A وأُصلح العيب.**
التسخين الآن يبني الرابط بنفس وسائط `ResponsiveMenuImage` تماماً، والمطابقة مؤكدة في التخطيطات الأربعة. **الأثر على الأداء ما زال غير مقاس** لأن حجب الشبكة لم يتغير. انظر قسم **Fix Applied**.
...
## Fix Applied — الخيار A (بعد موافقة المالك)

نُفِّذ الخيار A بعد الموافقة الصريحة. أقل تعديل ممكن، وبلا أي تغيير في الفكرة أو المعمار أو تدفّق البيانات.

| الملف | التغيير |
|---|---|
| `src/features/menu/MenuBody.jsx` | إضافة `width`/`height` لكل تخطيط في `LAYOUT_IMAGE_CONFIG` (circles 104×104، grid 240×240، showcase 480×360، list 108×108) — مطابقة لما يمرّره `ProductItem`. |
| `src/features/menu/imagePrefetch.js` | تمرير `{ width, height }` كوسيط رابع إلى `createSupabaseWebpSrcSet` (نفس استدعاء `ResponsiveMenuImage`)، وجعل `href` (fallback) يطابق أصغر مرشّح في الـsrcset. |
| `src/features/menu/imagePrefetch.test.js` | تصحيح التوقعات + **اختبار ربط جديد** يقارن ناتج التسخين بناتج `ResponsiveMenuImage` نفسه. |
| `qa/pr-03/prefetch-url-match-check.mjs` | تحويله إلى guard يغطي التخطيطات الأربعة ويخرج بالرمز 1 عند أي عدم تطابق. |

**لم يُمس:** `ResponsiveMenuImage.jsx`، `imageTransforms.js`، `useMenuData.js`، أي استعلام Supabase، أي schema/RLS، أي API contract، وأي feature أخرى.

### التحقق بعد الإصلاح

| الفحص | النتيجة |
|---|---|
| `node qa/pr-03/prefetch-url-match-check.mjs` | ✅ `MATCH in all layouts` (كان MISMATCH في الأربعة) |
| `npx vitest run` | ✅ **311/311** في 25 ملف (310 + الاختبار الجديد) |
| `npm run build` | ✅ نجح — `PublicMenu` 149.17 kB / gzip 39.16 kB |
| **اختبار أن الاختبار له أسنان** | ✅ أُعيد إدخال العيب عمداً → الاختبار **فشل** كما يجب، ثم أُعيد الإصلاح |

الفحص الأخير مهم: اختبار الربط لا يقارن بسلسلة ثابتة، بل يستدعي `ResponsiveMenuImage` فعلياً ويقارن `imageSrcset` بـ`source.props.srcSet`. أي تغيير مستقبلي في طريقة بناء الرابط داخل المكوّن سيُفشل الاختبار بدل أن يعطّل التسخين صامتاً — وهو بالضبط ما سمح للعيب الأصلي بالمرور.

### ما لم يتغير بالإصلاح

الإصلاح يجعل الآلية **صحيحة**، لا **مُثبَتة الأثر**. كل قيم FCP/LCP/CLS/TBT/First Product/Next Category Ready ما زالت `NOT AVAILABLE` لأن حجب الشبكة لم يتغير. الادعاء الوحيد المسموح به الآن هو: التسخين صار يطلب المورد نفسه الذي ستطلبه `<img>`، ومن ثم أصبح cache hit ممكناً بنيوياً — وهل يسبق المستخدم فعلاً على Slow 4G؟ **لم يُقَس بعد.**
...
| 5 | Prefetch يسبق المستخدم | ❌ **FAIL — مُثبَت** → بعد الإصلاح: ممكن بنيوياً، `NOT MEASURED` |
| 6 | لا Blank Images | `NOT MEASURED` |
| 7 | لا Network Flood | ⚠️ لا فيضان (سقف 3)، لكن 100% من الطلبات مهدورة → بعد الإصلاح: لم تعد مهدورة |
| 8 | لا Duplicate Requests | ❌ **FAIL — مُثبَت** → بعد الإصلاح: ✅ مورد واحد لكل صورة |
| 9 | لا Scroll Jank | `NOT MEASURED` |
| 10 | Cache يعمل | ⚠️ HTTP cache يعمل، لكن PR-03 لا يستفيد منه → بعد الإصلاح: ✅ يستفيد |
...
### بعد الإصلاح: `PARTIAL PASS`

**ما نجح (مُثبَت):** العيب الجذري أُزيل، وتطابق الروابط مؤكد في التخطيطات الأربعة، ولا تحميل مزدوج، والاختبارات 311/311، والبناء ناجح، وPR-01/PR-02 سليمان، ووُضع اختبار يمنع تكرار العيب صامتاً.

**ما لم يُثبَت بعد:** كل أرقام الأداء. لا يمكن ترقية الحكم إلى `PASS` — ولن أكتب `PR-03 CLOSED` — قبل تشغيل القياس على بيئة فيها شبكة. `PASS` بلا قياس سيكون بالضبط نمط «ASSUME → CLAIM SUCCESS» الممنوع في هذه المهمة.

---

## Recommendation

> **تحديث:** طُرح الخياران أدناه على المالك، و**اختار الخيار A ونُفِّذ بالفعل** (انظر Fix Applied). يبقى النص للتوثيق.

الخياران المطروحان وقتها:
...
**✅ تم: نُفِّذ الخيار A بالكامل، بما فيه اختبار الربط.** الخطوة المتبقية الوحيدة هي **القياس**.
...
| `qa/pr-03/prefetch-url-match-check.mjs` | guard تطابق الروابط في التخطيطات الأربعة |

**ملفات التطبيق المعدَّلة (بعد موافقة المالك على الخيار A فقط):**

| الملف | التغيير |
|---|---|
| `src/features/menu/MenuBody.jsx` | `width`/`height` في `LAYOUT_IMAGE_CONFIG` |
| `src/features/menu/imagePrefetch.js` | تمرير الأبعاد إلى بناء الرابط + href مطابق |
| `src/features/menu/imagePrefetch.test.js` | تصحيح التوقعات + اختبار ربط بالمستهلك |

لم يُنشأ PR-04، ولم تُضف أو تُحذف أي feature، ولم يُعد تصميم أي جزء.
