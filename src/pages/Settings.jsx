  ...
  useEffect(() => {
    if (restaurant) {
      setRestForm({
        ...
        allergens: Array.isArray(restaurant.allergens) ? restaurant.allergens : [],
                  ...
                  <div style={{ padding:'16px 18px' }}>
                    ...
                    <div style={{ display:'flex', flexDirection:'column', gap:'4px', marginBottom:'14px' }}>
                      {COMMON_ALLERGENS.map(a => {
                        const checked = restForm.allergens.includes(a.label)
                        return (
                              ...
                              onChange={e => {
                                setRestForm(f => ({
                                  ...
                                  allergens: e.target.checked
                                    ? [...f.allergens, a.label]
                                    : f.allergens.filter(x => x !== a.label),
                                }))
                              }}
                    ...
                    {restForm.allergens.filter(a => !COMMON_ALLERGENS.some(c => c.label === a)).map(a => (
                      <div key={a} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'8px 10px', background:'#FFF7ED', borderRadius:'8px', marginBottom:'6px' }}>
                        <span style={{ fontSize:'13px', fontWeight:'600' }}>⚠️ {a}</span>
                        <button onClick={() => setRestForm(f => ({ ...f, allergens: f.allergens.filter(x => x !== a) }))} style={{ background:'none', border:'none', color:'#EF4444', fontSize:'12px', cursor:'pointer', fontWeight:'700' }}>حذف</button>
                      </div>
                    ))}
                      ...
                      <input
                        ...
                        onKeyDown={e => {
                          if (e.key === 'Enter') {
                            ...
                            if (val && !restForm.allergens.includes(val)) {
                              setRestForm(f => ({ ...f, allergens: [...f.allergens, val] }))
                              e.target.value = ''
                            }
                          }
                        }}
