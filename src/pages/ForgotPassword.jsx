    ...
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'linear-gradient(135deg,#0B0B0F,#1a1a2e)', padding:'24px', direction:'rtl' }}>
      <div style={{ background:'white', borderRadius:'24px', padding:'48px 40px', width:'100%', maxWidth:'440px', boxShadow:'0 24px 64px rgba(0,0,0,0.3)' }}>
        ...
        {!sent ? <>
          ...
          <h2 style={{ fontFamily:'Tajawal,sans-serif', fontWeight:'900', fontSize:'24px', textAlign:'center', marginBottom:'8px' }}>نسيت كلمة المرور؟</h2>
          ...
          <form onSubmit={handle}>
            ...
            <button type="submit" disabled={loading} style={{ width:'100%', padding:'14px', borderRadius:'12px', border:'none', background:'linear-gradient(135deg,#FF6A00,#E05D00)', color:'white', fontFamily:'Tajawal,sans-serif', fontWeight:'800', fontSize:'15px', cursor:'pointer', opacity:loading?0.8:1 }}>
              {loading?'جارٍ الإرسال...':'إرسال رابط الاستعادة ←'}
            </button>
          </form>
        </> : <>
          <div style={{ textAlign:'center', fontSize:'64px', marginBottom:'16px' }}>✉️</div>
          <h2 style={{ fontFamily:'Tajawal,sans-serif', fontWeight:'900', fontSize:'22px', textAlign:'center', marginBottom:'8px' }}>تحقق من بريدك!</h2>
          <p style={{ fontSize:'14px', color:'#6B7280', textAlign:'center', lineHeight:'1.6' }}>أرسلنا رابط الاستعادة إلى<br/><strong style={{ color:'#FF6A00' }}>{email}</strong></p>
        </>}

        <div style={{ textAlign:'center', marginTop:'24px' }}>
          <Link to="/login" style={{ color:'#FF6A00', fontWeight:'700', fontSize:'14px' }}>→ العودة لتسجيل الدخول</Link>
        </div>
      </div>
    </div>
  )
