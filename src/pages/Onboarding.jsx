                            ...
                            <article key={key} style={{ padding:'13px', borderRadius:'16px', border:`1.5px solid ${selected ? '#FF6A00' : '#E5E7EB'}`, background:selected ? 'rgba(255,106,0,0.055)' : 'white', boxShadow:selected ? '0 6px 16px rgba(255,106,0,0.09)' : 'none', transition:'border-color 160ms ease-out, box-shadow 160ms ease-out, background-color 160ms ease-out' }}>
                              ...
                              {selected && (
                                <div dir="rtl" style={{ marginTop:'15px', paddingTop:'14px', borderTop:'1px solid #F2DDD1' }}>
                                  ...
                                  <div dir="ltr" style={{ display:'grid', gridTemplateColumns:'minmax(0, 1fr) auto', alignItems:'stretch', width:'100%', minWidth:0, minHeight:'48px', overflow:'hidden', border:`1.5px solid ${isValidItemPrice(price) ? '#CFE7D6' : '#F6C9B0'}`, borderRadius:'12px', background:'white' }}>
                                    <input id={`item-price-${key}`} type="number" min="0" step="0.01" inputMode="decimal" value={price} onChange={event => updateItemPrice(category, item, event.target.value)} placeholder="أدخل السعر" aria-label={`السعر الحقيقي لـ ${item.name}`} style={{ ...inputStyle, boxSizing:'border-box', width:'100%', minWidth:0, minHeight:'46px', padding:'11px 13px', direction:'ltr', textAlign:'left', fontSize:'16px', fontWeight:'800', color:'#111827', background:'transparent', border:'none', borderRadius:0, outline:'none' }} />
                                    <span aria-hidden="true" style={{ display:'flex', alignItems:'center', justifyContent:'center', minWidth:'58px', padding:'0 12px', borderInlineStart:'1px solid #E5E7EB', background:'#FAFBFC', color:'#4B5563', fontSize:'12px', fontWeight:'900', whiteSpace:'nowrap', pointerEvents:'none' }}>{rest?.currency || 'SAR'}</span>
                                  </div>
                                </div>
                              )}
                            </article>
