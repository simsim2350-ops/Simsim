...
export default function Orders() {
  ...
  // تعديل كمية صنف (قبل التحضير)
  const changeQty = async (order, i, delta) => {
    const items = (Array.isArray(order.items) ? order.items : []).map((it, idx) =>
      idx === i ? { ...it, qty: Math.max(1, (it.qty || 1) + delta) } : it)
    const { error } = await supabase.from('orders').update({ items, ...recalc(items) }).eq('id', order.id)
    if (error) toast.error(error.message)
  }

  // حذف صنف (قبل التحضير) — لو انحذف الأخير يُلغى الطلب
  const removeItem = async (order, i) => {
    const items = (Array.isArray(order.items) ? order.items : []).filter((_, idx) => idx !== i)
    if (items.length === 0) {
      await supabase.from('orders').update({ status:'cancelled', cancelled_by:'restaurant', cancel_reason:'حُذفت كل الأصناف' }).eq('id', order.id)
      setSelectedOrder(null)
      toast.error('🚫 تم إلغاء الطلب — لا أصناف متبقية')
      return
    }
    const { error } = await supabase.from('orders').update({ items, ...recalc(items) }).eq('id', order.id)
    if (error) toast.error(error.message)
  ...
  const toggleItemUnavailable = async (order, itemIndex) => {
    ...
    const newSubtotal = updatedItems.reduce((sum, it) => it.unavailable ? sum : sum + (it.price * it.qty), 0)
      ...
      <div
        ...
        onTouchEnd={(e) => {
          ...
          if (Math.abs(dx) > 70 && Math.abs(dy) < 40) {
            ...
            else if (isActive(order.status) && dx > 0) cancelOrder(order) // سحب لليمين = إلغاء
          }
        }}
            ...
            <div onClick={e => e.stopPropagation()} style={{ background:'white', width:'100%', maxWidth:'520px', maxHeight:'88vh', borderRadius:'20px 20px 0 0', overflowY:'auto', animation:'slideUp .25s ease' }}>
              ...
              <div style={{ padding:'16px 18px' }}>
                  ...
                  <div style={{ marginBottom:'12px' }}>
                    ...
                    {items.map((item, i) => (
                      <div key={i} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'6px 0', borderBottom:'1px solid #F0F0F0', opacity: item.unavailable ? 0.5 : 1 }}>
                        ...
                        <div style={{ display:'flex', alignItems:'center', gap:'8px' }}>
                          {order.status === 'pending' && !item.unavailable && (
                            <div style={{ display:'flex', alignItems:'center', gap:'2px', background:'#F3F4F6', borderRadius:'8px', padding:'2px' }}>
                              <button onClick={() => changeQty(order, i, -1)} style={{ width:'24px', height:'24px', borderRadius:'6px', border:'none', background:'white', cursor:'pointer', fontWeight:'800', fontSize:'14px', color:'#374151' }}>−</button>
                              <span style={{ minWidth:'20px', textAlign:'center', fontWeight:'800', fontSize:'12px' }}>{item.qty}</span>
                              <button onClick={() => changeQty(order, i, 1)} style={{ width:'24px', height:'24px', borderRadius:'6px', border:'none', background:'white', cursor:'pointer', fontWeight:'800', fontSize:'14px', color:'#374151' }}>+</button>
                            </div>
                          )}
                          {order.status === 'pending' && (
                            <>
                              <button onClick={() => toggleItemUnavailable(order, i)} title={item.unavailable ? 'إعادة' : 'غير متوفر'} style={{ padding:'5px 7px', borderRadius:'7px', border:`1.5px solid ${item.unavailable ? '#D1FAE5' : '#FEE2E2'}`, background: item.unavailable ? '#ECFDF5' : '#FEF2F2', color: item.unavailable ? '#065F46' : '#EF4444', fontSize:'11px', fontWeight:'700', cursor:'pointer', whiteSpace:'nowrap' }}>{item.unavailable ? '↺' : '⚠️'}</button>
                              <button onClick={() => removeItem(order, i)} title="حذف الصنف" style={{ padding:'5px 7px', borderRadius:'7px', border:'1.5px solid #FEE2E2', background:'#FEF2F2', color:'#EF4444', fontSize:'11px', fontWeight:'700', cursor:'pointer' }}>🗑</button>
                            </>
                          )}
                ...
                <div style={{ background:'#F8F9FB', borderRadius:'10px', padding:'10px 12px', marginBottom:'14px', border:'1px solid #E5E7EB' }}>
                  <div style={{ display:'flex', justifyContent:'space-between', fontSize:'12px', color:'#9CA3AF', marginBottom:'4px' }}><span>المجموع الجزئي</span><span>{Number(order.subtotal ?? order.total ?? 0).toFixed(2)} ﷼</span></div>
                  <div style={{ display:'flex', justifyContent:'space-between', fontSize:'12px', color:'#9CA3AF', marginBottom: order.delivery_fee > 0 ? '4px' : '6px' }}><span>الضريبة 15%</span><span>{Number(order.tax ?? 0).toFixed(2)} ﷼</span></div>
                  {order.delivery_fee > 0 && <div style={{ display:'flex', justifyContent:'space-between', fontSize:'12px', color:'#9CA3AF', marginBottom:'6px' }}><span>🛵 رسوم التوصيل</span><span>{Number(order.delivery_fee).toFixed(2)} ﷼</span></div>}
                  <div style={{ display:'flex', justifyContent:'space-between', fontFamily:'Cairo,sans-serif', fontWeight:'900', fontSize:'15px', paddingTop:'6px', borderTop:'1px solid #E5E7EB' }}><span>الإجمالي</span><span style={{ color:'#FF6B35' }}>{Number(order.total ?? 0).toFixed(2)} ﷼</span></div>
                </div>
