...
export default function Orders() {
  ...
  const filteredOrders = orders.filter(o => {
    if (branchFilter === '__none__' && o.branch_id) return false
    if (branchFilter !== 'all' && branchFilter !== '__none__' && o.branch_id !== branchFilter) return false
  ...
  return (
    <div style={{ display:'flex', height:'100vh', overflow:'hidden', direction:'rtl', position:'relative' }}>
      ...
      <main style={{ marginRight: isMobile ? '0' : '240px', flex:1, display:'flex', flexDirection:'column', height:'100vh', overflow:'hidden' }}>
        ...
        {/* Branch filter — يظهر فقط لو فيه فروع مضافة */}
        {branches.length > 0 && (
          <div style={{ background:'white', borderBottom:'1px solid #E5E7EB', padding:'10px 16px', flexShrink:0 }}>
            <select
              value={branchFilter}
              onChange={e => setBranchFilter(e.target.value)}
              style={{ padding:'7px 12px', borderRadius:'9px', border:'1.5px solid #E5E7EB', fontFamily:'Tajawal,sans-serif', fontSize:'12px', fontWeight:'700', color:'#374151', outline:'none', cursor:'pointer', background:'white' }}
            >
              <option value="all">🏢 كل الفروع</option>
              {branches.map(b => (
                <option key={b.id} value={b.id}>{b.name}</option>
              ))}
              <option value="__none__">بدون فرع محدد</option>
            </select>
          </div>
        )}
                      ...
                      <div style={{ padding:'12px 14px', display:'flex', alignItems:'center', gap:'10px' }}>
                        <div style={{ flex:1, minWidth:0 }}>
                          ...
                          <div style={{ fontSize:'12px', color:'#9CA3AF', display:'flex', gap:'8px', flexWrap:'wrap' }}>
                            ...
                            {order.branch_id && (() => {
                              const br = branches.find(b => b.id === order.branch_id)
                              return br ? <span>🏢 {br.name}</span> : null
                            })()}
