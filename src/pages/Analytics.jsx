...
export default function Analytics() {
  ...
  useEffect(() => {
    if (!restaurant) return
    fetchAnalytics()
  }, [restaurant, period, branchFilter])
  ...
  const fetchAnalytics = async () => {
    ...
    try {
      ...
      // فلتر الفرع: __main__ = الطلبات بدون فرع (الفرع الرئيسي)، أو فرع محدد
      if (branchFilter === '__main__') query = query.is('branch_id', null)
  ...
  return (
    <div style={{ display:'flex', height:'100vh', overflow:'hidden', direction:'rtl', position:'relative' }}>
      ...
      <main style={{ marginRight: isMobile?'0':'240px', flex:1, display:'flex', flexDirection:'column', height:'100vh', overflow:'hidden' }}>
        ...
        {/* Branch filter — يظهر فقط لو فيه فروع */}
        {branches.length > 0 && (
