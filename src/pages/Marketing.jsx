...
export default function Marketing() {
  ...
  const fetchAll = async () => {
    setLoading(true)
    try {
      const [{ data: b }, { data: c }, { data: br }] = await Promise.all([
            ...
            <>
              ...
              {banners.length === 0 ? (
                <div style={{ textAlign:'center', padding:'50px 16px', color:'#9CA3AF' }}>
                  ...
                  <div style={{ fontSize:'13px' }}>البانر العام يظهر في صفحة اختيار الفرع، والبانر الخاص بفرع محدد يظهر في منيو ذلك الفرع</div>
                </div>
              ) : (
            ...
            {branches.length > 0 && (
              <div style={{ marginBottom:'14px' }}>
                ...
                <div style={{ fontSize:'11px', color:'#9CA3AF', marginTop:'5px' }}>
                  لو اخترت فرعاً محدداً، البانر يظهر فقط في منيو ذلك الفرع (ليس في صفحة اختيار الفرع)
                </div>
              </div>
            )}
          ...
          <div style={{ background:'white', borderRadius:'24px 24px 0 0', width:'100%', maxWidth:'480px', padding:'20px', position:'relative', maxHeight:'85vh', overflowY:'auto' }}>
            ...
            {branches.length > 0 && (
              <div style={{ marginBottom:'14px' }}>
                ...
                <div style={{ fontSize:'11px', color:'#9CA3AF', marginTop:'5px' }}>
                  لو اخترت فرعاً محدداً، الكوبون يظهر فقط في منيو ذلك الفرع (ليس في صفحة اختيار الفرع)
                </div>
              </div>
            )}
