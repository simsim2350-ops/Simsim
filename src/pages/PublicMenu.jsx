...
function computeOpenStatus(hours) {
  ...
  if (!Array.isArray(hours) || hours.length !== 7) {
    return { open: true, unknown: true, todayText: '' }
  }
  const toMins = (t) => {
    if (!t) return null
  ...
  const todayText = today && today.open
    ? `${today.from} - ${today.to === '24:00' ? '00:00' : today.to}`
    : 'مغلق اليوم'

  return { open, unknown: false, todayText }
}
          ...
          <div style={{ display:'flex', alignItems:'flex-start', gap:'14px', marginBottom: restaurant.description ? '10px' : 0 }}>
            ...
            <div style={{ flex:1, paddingTop: restaurant.cover_url ? '38px' : 0 }}>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:'10px', flexWrap:'wrap' }}>
                {(() => {
                  const status = computeOpenStatus(branch?.opening_hours || restaurant.opening_hours)
                  const c = status.open ? '#10B981' : '#EF4444'
                  const bg = status.open ? 'rgba(16,185,129,0.1)' : 'rgba(239,68,68,0.1)'
                  return (
                    <>
                      <span style={{ display:'flex', alignItems:'center', gap:'4px', fontSize:'12px', fontWeight:'700', color:c, background:bg, padding:'3px 10px', borderRadius:'100px' }}>
                        ...
                        {status.open ? 'مفتوح الآن' : 'مغلق الآن'}
                      </span>
                      {!status.unknown && status.todayText && (
                        <span style={{ fontSize:'12px', color:'#9CA3AF', direction:'ltr' }}>🕐 {status.todayText}</span>
                      )}
                    </>
                  )
                })()}
