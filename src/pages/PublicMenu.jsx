    ...
    <div style={{ minHeight:'100vh', background:'#F8F9FB', direction:'rtl', fontFamily:'Tajawal,sans-serif', maxWidth:'480px', margin:'0 auto', position:'relative' }}>
      ...
      <div style={{ background:`linear-gradient(135deg, ${brandColor}22, ${brandColor}08)`, borderBottom:'1px solid #E5E7EB' }}>
        <div style={{ padding:'20px 16px 16px', display:'flex', alignItems:'center', gap:'14px' }}>
          <div style={{ width:'60px', height:'60px', borderRadius:'16px', background:`linear-gradient(135deg, ${brandColor}, ${brandColor}CC)`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:'28px', flexShrink:0, boxShadow:`0 4px 16px ${brandColor}44`, overflow:'hidden' }}>
            {restaurant.logo_url
        ...
        {!searchQuery && categories.map(cat => {
          ...
          return (
            <div key={cat.id} id={`cat-${cat.id}`} style={{ marginBottom:'8px' }}>
              {cat.cover_url && (
                <div style={{ height:'90px', margin:'0 16px 8px', borderRadius:'14px', overflow:'hidden' }}>
                  <img src={cat.cover_url} alt={cat.name} style={{ width:'100%', height:'100%', objectFit:'cover' }} />
                </div>
              )}
