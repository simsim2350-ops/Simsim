        ...
        <div style={{ padding: restaurant.cover_url ? '0 16px 16px' : '20px 16px 16px', marginTop: restaurant.cover_url ? '-32px' : 0 }}>
          ...
          {restaurant.social_links && Object.values(restaurant.social_links).some(v => v) && (
            <div style={{ display:'flex', gap:'8px' }}>
              {[
                { key:'instagram', icon:'📷' },
                { key:'whatsapp_social', icon:'💬' },
                { key:'snapchat', icon:'👻' },
                { key:'twitter', icon:'🐦' },
                { key:'tiktok', icon:'🎵' },
              ].filter(s => restaurant.social_links[s.key]).map(s => (
                <a
                  key={s.key}
                  href={restaurant.social_links[s.key]}
                  target="_blank"
                  rel="noopener noreferrer"
                  style={{ width:'34px', height:'34px', borderRadius:'50%', background:'white', border:'1.5px solid #E5E7EB', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'16px', textDecoration:'none', boxShadow:'0 2px 6px rgba(0,0,0,0.06)' }}
                >
                  {s.icon}
                </a>
              ))}
          ...
          <div style={{ marginBottom:'8px' }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:'1px', background:'#F3F4F6' }}>
              {bestSellers.map(prod => (
                <ProductItem key={prod.id} product={prod} cart={cart} onAdd={() => { setSelectedProduct(prod); setModalQty(1); setModalNote(''); setModalOptions({}) }} onQtyChange={(delta) => delta > 0 ? addToCart(prod, 1) : removeFromCart(`${prod.id}____`)} brandColor={brandColor} />
              ))}
            </div>
          </div>
        )}
