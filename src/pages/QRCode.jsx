...
export default function QRCodePage() {
  ...
  const [selectedBranch, setSelectedBranch] = useState('main') // 'main' = الفرع الرئيسي (بدون باراميتر)
  ...
  const menuURL = restaurant
    ? `${window.location.origin}/menu/${restaurant.slug}${activeBranch ? `?branch=${activeBranch.id}` : ''}`
    : ''
  ...
  }, [restaurant, qrColor, qrSize, menuURL])
  ...
  const downloadQR = () => {
    ...
    link.download = `qr-${restaurant?.slug || 'menu'}${activeBranch ? `-${activeBranch.id.slice(0,8)}` : ''}.png`
  ...
  const downloadCardImage = async () => {
    ...
    if (activeBranch) {
      ...
      ctx.fillText('🏢 ' + activeBranch.name, W/2, 240)
      ctx.globalAlpha = 1
    }
    ...
    ctx.fillText('امسح للاطلاع على المنيو وتقديم طلبك', W/2, activeBranch ? 268 : 250)
    ctx.globalAlpha = 1
        ...
        <div style={{ flex:1, overflowY:'auto', padding:'16px' }}>
          <div style={{ display:'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap:'16px', maxWidth:'900px', margin:'0 auto' }}>
                ...
                <div style={{ padding:'24px', display:'flex', justifyContent:'center' }}>
                  <div style={{ width:'260px', borderRadius:'20px', overflow:'hidden', boxShadow:'0 8px 32px rgba(0,0,0,0.15)' }}>
                    ...
                    <div style={{ background: style.cardBg, padding:'20px 20px 14px', display:'flex', flexDirection:'column', alignItems:'center', gap:'10px' }}>
                      ...
                      {activeBranch && (
                        <div style={{ fontFamily:'Cairo,sans-serif', fontWeight:'700', fontSize:'12px', color: style.textColor, opacity:0.92, textAlign:'center' }}>
                          🏢 {activeBranch.name}
                        </div>
                      )}
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:'14px' }}>
              ...
              {/* Branch selector — يظهر فقط لو فيه فروع */}
              {branches.length > 0 && (
                <div style={{ background:'white', borderRadius:'16px', border:'1px solid #E5E7EB', overflow:'hidden' }}>
                  ...
                  <div style={{ padding:'14px 16px' }}>
                    <select
                      ...
                      style={{ width:'100%', padding:'10px 12px', borderRadius:'10px', border:'1.5px solid #E5E7EB', fontFamily:'Tajawal,sans-serif', fontSize:'13px', fontWeight:'700', color:'#374151', outline:'none', cursor:'pointer', background:'white' }}
                    >
                      <option value="main">🏠 الفرع الرئيسي (المنيو العام)</option>
                      {branches.map(b => (
                        <option key={b.id} value={b.id}>🏢 {b.name}</option>
                      ))}
                    </select>
                    <div style={{ fontSize:'11px', color:'#9CA3AF', marginTop:'8px', lineHeight:'1.6' }}>
                      كل فرع له QR خاص — الطلبات اللي تجي منه تنربط بالفرع تلقائياً وتظهر في الطلبات والتحليلات.
                    </div>
                  </div>
                </div>
              )}
