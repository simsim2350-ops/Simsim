...
export default function Dashboard() {
  ...
  const shareMenu = async () => { const url = `${appConfig.menuNextBaseUrl}/menu/${restaurant?.slug}`; try { await navigator.clipboard.writeText(url); trackOwnerMilestone('menu_link_copied', { restaurantId: restaurant?.id, props: { source: 'dashboard' } }); toast.success('تم نسخ رابط المنيو') } catch { toast.error('تعذر نسخ الرابط') } }
  ...
  return <AppShell active="dashboard" title="الرئيسية" badges={{ orders: activeOrders.length }} actions={<div className="topbar-date">{new Date().toLocaleDateString('ar-SA', { weekday: 'long', day: 'numeric', month: 'long' })}</div>}>
    <div className="dash2"><div className="container">
      <header className="hero"><div><h1>مرحبًا {firstName}</h1><p className="sub">إليك ملخص أداء {restaurant?.name || 'مطعمك'} اليوم، وما يحتاج انتباهك الآن.</p></div><div className="hero-actions"><div className="store-state"><span className="dot" /> المطعم مفتوح ويستقبل الطلبات</div><button className="button" onClick={() => window.open(`${appConfig.menuNextBaseUrl}/menu/${restaurant?.slug}`, '_blank')}><Icon name="external" size={15} /> معاينة المنيو</button><button className="button" onClick={() => go('/qr')}><Icon name="qr" size={15} /> QR Code</button><button className="button primary" onClick={shareMenu}><Icon name="share" size={15} /> مشاركة المنيو</button></div></header>
      ...
      {error ? <div className="panel"><ErrorState retry={loadData} /></div> : <>
        ...
        <div className="stack">
          ...
          <Section title="إجراءات سريعة" eyebrow="اختصارات العمل"><div className="quick"><button className="primary" onClick={() => go('/menu', { tab: 'products' })}><Icon name="plus" size={18} />إضافة صنف</button><button onClick={() => go('/menu')}><Icon name="menu" size={18} />إدارة المنيو</button><button onClick={() => window.open(`${appConfig.menuNextBaseUrl}/menu/${restaurant?.slug}`, '_blank')}><Icon name="external" size={18} />معاينة المنيو</button><button onClick={() => go('/qr')}><Icon name="qr" size={18} />QR Code</button><button onClick={() => go('/analytics')}><Icon name="chart" size={18} />التقرير</button><button onClick={shareMenu}><Icon name="share" size={18} />مشاركة المنيو</button></div></Section>
        </div></div>
      </>}
    </div></div>
  </AppShell>
