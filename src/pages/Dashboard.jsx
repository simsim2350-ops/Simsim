    ...
    <div className="dash2"><div className="container">
      ...
      {error ? <div className="panel"><ErrorState retry={loadData} /></div> : <>
        <div className="kpis">{loading ? [1, 2, 3, 4].map(i => <Skeleton key={i} className="k" />) : <>
          {[{ label: 'مبيعات اليوم', value: todayRevenue ? money(todayRevenue) : 'لا توجد مبيعات اليوم', note: `مقارنة بـ ${money(yesterdayRevenue)} أمس`, icon: 'wallet', accent: '#f26622', trend: percent(todayRevenue, yesterdayRevenue) }, { label: 'طلبات اليوم', value: todayOrders.length || 'لا توجد طلبات اليوم', note: `${activeOrders.length} طلبات تحتاج إجراء`, icon: 'cart', accent: '#3578d4', trend: percent(todayOrders.length, yesterdayOrders.length) }, { label: 'متوسط قيمة الطلب', value: averageOrder ? money(averageOrder) : 'لا توجد طلبات مكتملة', note: `${completed.length} طلبات مكتملة`, icon: 'chart', accent: '#139a68' }, { label: 'العملاء', value: customers || 'لا يوجد عملاء بعد', note: avgRating ? `متوسط التقييم ${avgRating} من 5` : 'لا توجد تقييمات بعد', icon: 'users', accent: '#7b61d1' }].map(kpi => <div className="kpi" style={{ '--accent': kpi.accent }} key={kpi.label}><div className="kpi-top"><div className="kpi-icon"><Icon name={kpi.icon} size={18} /></div>{kpi.trend !== undefined && <span className={`trend ${kpi.trend < 0 ? 'down' : ''}`}>{kpi.trend >= 0 ? '↑' : '↓'} {Math.abs(kpi.trend)}%</span>}</div><div className="kpi-value">{kpi.value}</div><div className="kpi-label">{kpi.label}</div><div className="kpi-note">{kpi.note}</div></div>)}</>}</div>
        ...
        <div className="stack">
          ...
          <Section title="الأكثر مبيعًا" eyebrow="أداء المنيو" action="عرض تقرير المنيو" onAction={() => go('/analytics')}>
            {summaryLoading ? <div className="product-list">{[1, 2, 3].map(i => <Skeleton key={i} className="row-s" />)}</div> : topProducts.length ? <div className="product-list">{topProducts.map((p, i) => <div className="product-row" key={p.name}><span className="rank">{i + 1}</span><span className="product-name truncate">{p.name}</span><span className="product-meta"><b>{Number(p.count || 0).toLocaleString('ar-SA')}</b> طلب</span><span className="product-meta"><b>غير متاح</b> الإيراد</span></div>)}</div> : <EmptyState icon="menu" title="لا توجد مبيعات أصناف بعد" text="ستظهر الأصناف الأكثر مبيعًا بعد أول الطلبات." />}
          </Section>
          <Section title="صحة المنيو" eyebrow="جودة البيانات" action="تحسين المنيو" onAction={() => go('/menu')}><div className="health"><div className="health-score"><strong>{menu.products.length ? `${health}%` : 'غير متاح'}</strong><span>{menu.products.length ? 'مكتمل' : 'أضف أصنافًا لقياس الصحة'}</span></div><div className="progress"><span style={{ width: `${menu.products.length ? health : 0}%` }} /></div><div className="checks">{healthChecks.map(check => <div className={`check ${check.ok ? 'ok' : 'warn'}`} key={check.label}><Icon name={check.ok ? 'check' : 'alert'} size={14} /> {check.label}</div>)}</div></div></Section>
