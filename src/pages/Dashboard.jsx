...
export default function Dashboard() {
  ...
  const subscribeReviews = () => {
    ...
    const ch = supabase.channel('reviews-dash')
      .on('postgres_changes',
        { event:'INSERT', schema:'public', table:'reviews', filter:`restaurant_id=eq.${restaurant.id}` },
        (p) => {
          setReviews(prev => [p.new, ...prev])
          toast.success(`⭐ تقييم جديد: ${p.new.rating}/5`)
        }
      ).subscribe()
      ...
      <div style={{
        ...
        <aside style={{ width:'240px', background:'#0F1117', height:'100dvh', display:'flex', flexDirection:'column', borderLeft:'1px solid rgba(255,255,255,0.06)', overflowY:'auto' }}>
          ...
          <nav style={{ padding:'8px 12px', flex:1, overflowY:'auto' }}>
            ...
            <NavItem icon="🍽️" label="الأصناف" onClick={() => { navigate('/menu', { state: { tab: 'products' } }); setSidebarOpen(false) }} />
            ...
            <div style={{ padding:'14px 16px', borderBottom:'1px solid #E5E7EB', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
              ...
              {reviews.length > 0 && (
                <span style={{ fontSize:'12px', fontWeight:'800', color:'#F59E0B' }}>
                  ...
                  <span style={{ color:'#9CA3AF', fontWeight:'600', marginRight:'6px' }}>({reviews.length})</span>
                </span>
              )}
            </div>
            {reviews.length === 0 ? (
              <div style={{ padding:'32px 16px', textAlign:'center', color:'#9CA3AF' }}>
                ...
                <div style={{ fontSize:'13px' }}>لا توجد تقييمات بعد — ستظهر هنا بعد أن يقيّم العملاء طلباتهم المكتملة</div>
              </div>
            ) : (
                  ...
                  <div key={r.id} style={{ padding:'12px 16px', borderBottom:'1px solid #F3F4F6' }}>
                    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom: r.comment ? '6px' : 0 }}>
                      <div style={{ display:'flex', alignItems:'center', gap:'8px', minWidth:0 }}>
                        <span style={{ fontSize:'13px', fontWeight:'800', color:'#F59E0B', whiteSpace:'nowrap' }}>
                          {'⭐'.repeat(r.rating || 0)}
                        </span>
                        <span style={{ fontSize:'12px', color:'#374151', fontWeight:'700', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                          {r.customer_name || 'عميل'}
                        </span>
                      </div>
                      <span style={{ fontSize:'11px', color:'#9CA3AF', flexShrink:0 }}>
                        {new Date(r.created_at).toLocaleDateString('ar', { day:'numeric', month:'short' })}
                      </span>
                    </div>
                    {r.comment && (
                      <div style={{ fontSize:'13px', color:'#0F1117', lineHeight:'1.6', background:'#F8F9FB', borderRadius:'10px', padding:'8px 11px' }}>
                        {r.comment}
                      </div>
                    )}
                    {r.customer_phone && (
                      <div style={{ fontSize:'11px', color:'#9CA3AF', marginTop:'5px', direction:'ltr', textAlign:'right' }}>📱 {r.customer_phone}</div>
                    )}
                  </div>
                ))}
              </div>
