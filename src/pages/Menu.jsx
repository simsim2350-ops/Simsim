...
export default function Menu() {
  ...
  const [prodForm, setProdForm] = useState({ name:'', description:'', price:'', category_id:'', emoji:'🍽️', is_available:true, is_featured:false })
  ...
  const saveCat = async () => {
    if (!catForm.name) { toast.error('أدخل اسم القسم'); return }
    try {
      if (editingCat) {
  ...
  const openAddProd = () => {
    setEditingProd(null)
    setProdForm({ name:'', description:'', price:'', category_id: categories[0]?.id || '', emoji:'🍽️', is_available:true, is_featured:false })
    setProdModal(true)
  }

  const openEditProd = (prod) => {
    setEditingProd(prod)
    setProdForm({ name:prod.name, description:prod.description||'', price:prod.price, category_id:prod.category_id||'', emoji:prod.emoji||'🍽️', is_available:prod.is_available, is_featured:prod.is_featured })
    setProdModal(true)
  }

  const saveProd = async () => {
    if (!prodForm.name) { toast.error('أدخل اسم الصنف'); return }
  ...
  return (
    <div style={{ display:'flex', height:'100vh', overflow:'hidden', direction:'rtl', position:'relative' }}>
      ...
      <div style={{
        ...
        transform: window.innerWidth > 768 ? 'none' : sidebarOpen ? 'translateX(0)' : 'translateX(100%)',
        transition:'transform 0.3s ease',
        zIndex:50, height:'100vh',
      }}>
        <aside style={{ width:'240px', background:'#0F1117', height:'100vh', display:'flex', flexDirection:'column', borderLeft:'1px solid rgba(255,255,255,0.06)' }}>
      ...
      {/* Main */}
      <main style={{ marginRight: window.innerWidth > 768 ? '240px' : '0', flex:1, display:'flex', flexDirection:'column', height:'100vh', overflow:'hidden' }}>
        ...
        <div style={{ height:'56px', background:'white', borderBottom:'1px solid #E5E7EB', display:'flex', alignItems:'center', padding:'0 16px', gap:'10px', flexShrink:0 }}>
          <button onClick={() => setSidebarOpen(!sidebarOpen)} style={{ background:'none', border:'none', fontSize:'22px', cursor:'pointer', display: window.innerWidth > 768 ? 'none' : 'block' }}>☰</button>
          <span style={{ fontSize:'16px', fontWeight:'800' }}>إدارة المنيو</span>
          <div style={{ marginRight:'auto', display:'flex', gap:'8px' }}>
            ...
            >
              ＋ {tab === 'categories' ? 'قسم جديد' : 'صنف جديد'}
            </button>
          </div>
        </div>
            ...
            <div>
              {categories.length === 0 ? (
                <div style={{ textAlign:'center', padding:'60px 16px', color:'#9CA3AF' }}>
                  ...
                  <div style={{ fontSize:'16px', fontWeight:'700', marginBottom:'8px' }}>لا توجد أقسام بعد</div>
                    ...
                    <div key={cat.id} style={{ background:'white', borderRadius:'14px', border:'1.5px solid #E5E7EB', padding:'14px 16px', display:'flex', alignItems:'center', gap:'12px' }}>
                      ...
                      <div style={{ display:'flex', alignItems:'center', gap:'8px' }}>
                        ...
                        <button
                          ...
                          style={{ padding:'5px 10px', borderRadius:'8px', border:'1.5px solid #E5E7EB', background: cat.is_visible ? '#D1FAE5' : '#F3F4F6', color: cat.is_visible ? '#065F46' : '#6B7280', fontSize:'11px', fontWeight:'700', cursor:'pointer' }}
                        >
                          {cat.is_visible ? '👁️ مرئي' : '🚫 مخفي'}
                        </button>
            ...
            <div>
              {products.length === 0 ? (
                <div style={{ textAlign:'center', padding:'60px 16px', color:'#9CA3AF' }}>
                  ...
                  <div style={{ fontSize:'16px', fontWeight:'700', marginBottom:'8px' }}>لا توجد أصناف بعد</div>
                  ...
                  {products.map(prod => (
                    <div key={prod.id} style={{ background:'white', borderRadius:'14px', border:'1.5px solid #E5E7EB', padding:'14px 16px', display:'flex', alignItems:'center', gap:'12px' }}>
                      ...
                      <div style={{ flex:1, minWidth:0 }}>
                        ...
                        <div style={{ fontSize:'12px', color:'#9CA3AF', marginBottom:'4px', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{prod.description}</div>
                        <div style={{ display:'flex', alignItems:'center', gap:'8px' }}>
                          ...
                          {prod.is_featured && <span style={{ fontSize:'11px', color:'#92400E', background:'#FEF3C7', padding:'2px 7px', borderRadius:'100px' }}>⭐ مميز</span>}
                        </div>
                      </div>
                      <div style={{ display:'flex', flexDirection:'column', gap:'6px', alignItems:'flex-end' }}>
                        <button
                          onClick={() => toggleProdAvailability(prod)}
                          style={{ padding:'4px 8px', borderRadius:'7px', border:'1.5px solid #E5E7EB', background: prod.is_available ? '#D1FAE5' : '#F3F4F6', color: prod.is_available ? '#065F46' : '#6B7280', fontSize:'10px', fontWeight:'700', cursor:'pointer', whiteSpace:'nowrap' }}
                        >
      ...
      {catModal && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:200, display:'flex', alignItems:'flex-end', justifyContent:'center' }} onClick={() => setCatModal(false)}>
          <div style={{ background:'white', borderRadius:'24px 24px 0 0', width:'100%', maxWidth:'480px', padding:'24px', paddingBottom:'36px' }} onClick={e => e.stopPropagation()}>
            <div style={{ width:'40px', height:'4px', background:'#E5E7EB', borderRadius:'2px', margin:'0 auto 20px' }}/>
            <h3 style={{ fontFamily:'Cairo,sans-serif', fontWeight:'900', fontSize:'18px', marginBottom:'20px', textAlign:'center' }}>
              {editingCat ? 'تعديل القسم' : 'إضافة قسم جديد'}
            </h3>

            <div style={{ marginBottom:'14px' }}>
              <label style={{ display:'block', fontSize:'13px', fontWeight:'700', marginBottom:'6px' }}>أيقونة القسم</label>
              <div style={{ display:'flex', gap:'8px', flexWrap:'wrap' }}>
                {EMOJIS.map(e => (
                  <div key={e} onClick={() => setCatForm(f=>({...f,emoji:e}))} style={{ width:'38px', height:'38px', borderRadius:'9px', border:`2px solid ${catForm.emoji===e?'#FF6B35':'#E5E7EB'}`, background: catForm.emoji===e?'#FFF0EB':'white', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'20px', cursor:'pointer' }}>
                    {e}
                  </div>
                ))}
              </div>
            ...
            <div style={{ marginBottom:'14px' }}>
              <label style={{ display:'block', fontSize:'13px', fontWeight:'700', marginBottom:'6px' }}>اسم القسم *</label>
              <input style={inputStyle} placeholder="مثال: البرغر، المشروبات..." value={catForm.name} onChange={e => setCatForm(f=>({...f,name:e.target.value}))} />
            </div>
      ...
      {prodModal && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:200, display:'flex', alignItems:'flex-end', justifyContent:'center' }} onClick={() => setProdModal(false)}>
          <div style={{ background:'white', borderRadius:'24px 24px 0 0', width:'100%', maxWidth:'480px', maxHeight:'90vh', overflowY:'auto', padding:'24px', paddingBottom:'36px' }} onClick={e => e.stopPropagation()}>
            <div style={{ width:'40px', height:'4px', background:'#E5E7EB', borderRadius:'2px', margin:'0 auto 20px' }}/>
            <h3 style={{ fontFamily:'Cairo,sans-serif', fontWeight:'900', fontSize:'18px', marginBottom:'20px', textAlign:'center' }}>
              {editingProd ? 'تعديل الصنف' : 'إضافة صنف جديد'}
            </h3>

            <div style={{ marginBottom:'14px' }}>
              <label style={{ display:'block', fontSize:'13px', fontWeight:'700', marginBottom:'6px' }}>أيقونة الصنف</label>
              <div style={{ display:'flex', gap:'8px', flexWrap:'wrap' }}>
                {EMOJIS.map(e => (
                  <div key={e} onClick={() => setProdForm(f=>({...f,emoji:e}))} style={{ width:'38px', height:'38px', borderRadius:'9px', border:`2px solid ${prodForm.emoji===e?'#FF6B35':'#E5E7EB'}`, background: prodForm.emoji===e?'#FFF0EB':'white', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'20px', cursor:'pointer' }}>
                    {e}
                  </div>
                ))}
              </div>
            ...
            <div style={{ marginBottom:'14px' }}>
              <label style={{ display:'block', fontSize:'13px', fontWeight:'700', marginBottom:'6px' }}>اسم الصنف *</label>
              <input style={inputStyle} placeholder="مثال: برغر كلاسيك" value={prodForm.name} onChange={e => setProdForm(f=>({...f,name:e.target.value}))} />
            </div>

            <div style={{ marginBottom:'14px' }}>
              <label style={{ display:'block', fontSize:'13px', fontWeight:'700', marginBottom:'6px' }}>الوصف</label>
              <textarea style={{ ...inputStyle, minHeight:'72px', resize:'vertical' }} placeholder="وصف الصنف..." value={prodForm.description} onChange={e => setProdForm(f=>({...f,description:e.target.value}))} />
            </div>

            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px', marginBottom:'14px' }}>
              <div>
                <label style={{ display:'block', fontSize:'13px', fontWeight:'700', marginBottom:'6px' }}>السعر (ريال) *</label>
                <input style={{ ...inputStyle, direction:'ltr', textAlign:'left' }} type="number" placeholder="0.00" value={prodForm.price} onChange={e => setProdForm(f=>({...f,price:e.target.value}))} />
              </div>
              <div>
                <label style={{ display:'block', fontSize:'13px', fontWeight:'700', marginBottom:'6px' }}>القسم</label>
