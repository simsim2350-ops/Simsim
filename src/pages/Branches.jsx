...
// أوقات العمل الافتراضية (الأحد = 0، مطابق لـ JS Date.getDay)
const DEFAULT_HOURS = [
  ...
  { day:'السبت',   open:true,  from:'09:00', to:'23:00' },
]
...
export default function Branches() {
  ...
  const [form, setForm] = useState({ name:'', address:'', phone:'', maps_url:'', is_active:true, opening_hours: DEFAULT_HOURS })
  ...
  const openAdd = () => {
    ...
    setForm({ name:'', address:'', phone:'', maps_url:'', is_active:true, opening_hours: DEFAULT_HOURS })
    setModalOpen(true)
  }
  ...
  const openEdit = (branch) => {
    ...
    setForm({
      ...
      opening_hours: Array.isArray(branch.opening_hours) && branch.opening_hours.length === 7
  ...
  const updateHour = (i, field, val) => {
    setForm(f => ({
      ...f,
      opening_hours: f.opening_hours.map((h, idx) => idx === i ? { ...h, [field]: val } : h),
    }))
  }
    ...
    try {
      if (editingBranch) {
        const { error } = await supabase.from('branches')
          .update({ name:form.name, address:form.address, phone:form.phone, maps_url:form.maps_url, is_active:form.is_active, opening_hours:form.opening_hours })
      ...
      {modalOpen && (
        <div style={{ position:'fixed', inset:0, zIndex:100, display:'flex', alignItems:'flex-end', justifyContent:'center' }}>
          ...
          <div style={{ background:'white', borderRadius:'24px 24px 0 0', width:'100%', maxWidth:'480px', padding:'20px', position:'relative', maxHeight:'85vh', overflowY:'auto' }}>
            ...
            {/* أوقات العمل — خاصة بهذا الفرع */}
            <div style={{ marginBottom:'18px' }}>
              <label style={labelStyle}>🕐 أوقات العمل</label>
              <div style={{ display:'flex', flexDirection:'column', gap:'7px' }}>
                {form.opening_hours.map((h, i) => (
                  <div key={h.day} style={{ display:'flex', alignItems:'center', gap:'8px', padding:'8px 10px', borderRadius:'10px', border:'1.5px solid #E5E7EB', background: h.open ? 'white' : '#F8F9FB' }}>
                    ...
                    <span style={{ fontSize:'12px', fontWeight:'700', width:'48px', flexShrink:0, color: h.open ? '#0F1117' : '#9CA3AF' }}>{h.day}</span>
                    {h.open ? (
                      <div style={{ display:'flex', alignItems:'center', gap:'5px', flex:1, justifyContent:'flex-end', direction:'ltr' }}>
                        <input type="time" value={h.from} onChange={e => updateHour(i,'from',e.target.value)} style={{ padding:'4px 5px', border:'1.5px solid #E5E7EB', borderRadius:'7px', fontFamily:'Tajawal,sans-serif', fontSize:'12px', outline:'none', width:'86px' }}/>
                        <span style={{ color:'#9CA3AF', fontSize:'12px' }}>—</span>
                        <input type="time" value={h.to} onChange={e => updateHour(i,'to',e.target.value)} style={{ padding:'4px 5px', border:'1.5px solid #E5E7EB', borderRadius:'7px', fontFamily:'Tajawal,sans-serif', fontSize:'12px', outline:'none', width:'86px' }}/>
                      </div>
                    ) : (
                      <span style={{ fontSize:'12px', color:'#9CA3AF', flex:1 }}>مغلق</span>
                    )}
                  </div>
                ))}
              </div>
              <div style={{ fontSize:'11px', color:'#9CA3AF', marginTop:'8px', lineHeight:'1.6' }}>
                تظهر في منيو هذا الفرع كـ "مفتوح الآن / مغلق الآن" حسب الوقت. للفترات الممتدة بعد منتصف الليل استخدم مثلاً 18:00 — 02:00.
              </div>
            </div>
