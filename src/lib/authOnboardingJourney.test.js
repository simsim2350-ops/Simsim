...
describe('auth and onboarding journey contract', () => {
  it('يوحّد تهيئة INITIAL_SESSION وSIGNED_IN في bootstrap واحد مع حالات تحميل صريحة', () => {
    ...
    expect(authStore).toContain("bootstrapStage: 'AUTH_SESSION'")
    expect(authStore).toContain("bootstrapStage: 'OWNER'")
    expect(authStore).toContain("bootstrapStage: 'RESTAURANT'")
    expect(authStore).toContain("bootstrapStage: 'READY'")
    expect(authStore).toContain("bootstrapStage: 'ERROR'")
    expect(authStore).toContain("operation: 'restaurant_context'")
  ...
  it('يوحّد تقرير الوجهة بعد callback وتسجيل الدخول والمسار العام', () => {
    ...
    expect(app).toContain("authState === 'ERROR'")
  ...
  it('يعرض Onboarding خطأ قابلًا لإعادة المحاولة ويحمي إنشاء المطعم والمنيو من التكرار', () => {
    ...
    expect(onboarding).toContain('data-onboarding-phase={initializationPhase}')
    expect(onboarding).toContain("operation: 'onboarding_restaurant'")
    expect(onboarding).toContain("operation: 'onboarding_categories'")
    expect(onboarding).toContain("setInitializationPhase('ERROR')")
  ...
  it('يدير أقسام Onboarding الحقيقية عبر الفرع الرئيسي ويستعيدها عند الاستئناف', () => {
    ...
    expect(onboarding).toContain("operation: 'onboarding_primary_branch'")
    expect(onboarding).toContain(".from('categories')")
    expect(onboarding).toContain(".eq('branch_id', branch.id)")
    expect(onboarding).toContain("await loadOnboardingCategories(r, { failFast: true })")
