...
- **[ADR-15] Sticky Header بأسلوب Morph (يوليو 2026):** تطبيق المبدأ السادس من فلسفة تصميم البطاقة. نفس بطاقة الهوية العائمة (وليس هيدر منفصل جديد) تتحوّل تدريجياً عبر انتقالات CSS (`transition`, بلا مكتبة رسوم متحركة) لنسخة مصغّرة تلتصق أعلى الشاشة (`position: sticky`) بعد تجاوز التمرير حداً معيناً (130px)، وتعود لحجمها الطبيعي تحت حد أدنى (70px) — فارق التمرير (hysteresis) يمنع الرفرفة عند التمرير القريب من الحد. حالة `isCompact` تُحسب بمستمع `scroll` مُقيَّد بـ`requestAnimationFrame` (بلا مكتبة). النسخة المصغّرة تُبقي **شعار (مصغّر) + اسم المطعم + التقييم ★** فقط (قرار صريح من المالك)، وتُخفي الباقي (الفرع، مقبض السحب/البحث، الوصف، الموقع، التواصل/المسبّبات، بطاقة الإحصائيات، بانر الولاء) بانكماش سلس (`max-height` + `opacity`). كل التعديل داخل `MenuHeader.jsx` فقط — لا تغيير في قاعدة البيانات ولا في مكوّنات أخرى.
